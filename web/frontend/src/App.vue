    ...
    <el-container>
      <el-header style="height: 60px;width: 100%">
        <el-row v-if="user.is_login">
          ...
          <el-col :span="1">
            <el-avatar v-if="imageURL" fit="cover" :size="70" :src="imageURL" lazy>
            </el-avatar>
            <el-avatar v-else :size="70" icon="el-icon-user-solid">
            </el-avatar>
          </el-col>
          <el-col :span="1">
        ...
        <el-row v-else>
          <el-col :span="4" offset="20" style="margin-top: 12px">
            ...
            <el-dialog @close="clear" title="登录" :visible.sync="dialogFormVisible">
              <el-form :model="form">
                <el-form-item label="用户名" :label-width="formLabelWidth">
                  <el-input v-model="form.name" placeholder="请输入中文，字母，数字的组合" autocomplete="off"
                            @keypress.enter.native.stop="login"/>
                </el-form-item>
                <el-form-item label="密码" :label-width="formLabelWidth">
                  <el-input show-password v-model="form.password" placeholder="请输入字母，数字的组合" autocomplete="off"
                            @keypress.enter.native.stop="login"/>
                </el-form-item>
              </el-form>
            ...
            <el-button style="margin-right: -20px" type="success" @click="dialogFormVisible1 = true">注册</el-button>
            <el-dialog @close="clear1" title="注册" :visible.sync="dialogFormVisible1">
              <el-form :model="form1">
                <el-form-item label="用户名" :label-width="formLabelWidth">
                  <el-input v-model="form1.name" placeholder="请输入中文，字母，数字的组合" autocomplete="off"
                            @keypress.enter.native="register"/>
                </el-form-item>
                <el-form-item label="密码" :label-width="formLabelWidth">
                  <el-input show-password v-model="form1.password" autocomplete="off" placeholder="请输入字母，数字的组合"
                            @keypress.enter.native="register"/>
                </el-form-item>
              </el-form>
              <div slot="footer" class="dialog-footer">
      ...
      <el-container>
        <el-aside width="200px" style="margin-top: 120px">
          <router-view name="menu" @first="first" :find="is_find" :cty="is_community" @Back1="fnd"
                       @Back2="cty"/>
        </el-aside>
        <el-main style="margin-top: 70px">
          <img src="./assets/logo.png" width="30%">
          <router-view v-if="is_find" name="find" :login="this.user.is_login" @show="show_books"
                       :books="booklists"/>
          <router-view ref=community v-if="is_community" @answer="reply" :user="this.user.name" @del="del_post"
                       @del_reply="del_reply" :login="this.user.is_login" :tableData="Data"
                       name="community" @post="Post" @show="show_post"/>
          <router-view v-if="is_first" @addbook="add_book" @delbook="del_book" name="book" @Download="download"
                       :books="booklist" :msg="msg" :login="this.user.is_login" @show="show_book"/>
      ...
      <div>
        <el-button @click="go_top" style="position: fixed;bottom: 60px;right: 180px" id="to_top" type="success"
                   size="small"
      ...
      <el-footer id="footer">
        ...
        <div align="right"><span style="font-size: 12px">github地址：<a href="https://github.com/zyh66615"
                                                                     style="font-size: 12px">zyh66615</a></span></div>
      </el-footer>
    </el-container>
  </div>
</template>
...
export default {
  ...
  created () {
    if (sessionStorage.getItem('userMsg')) {
      ...
      this.user.is_login = JSON.parse(sessionStorage.getItem('userMsg')).App.is_login
      this.booklist = JSON.parse(sessionStorage.getItem('userMsg')).App.booklist
      this.booklists = JSON.parse(sessionStorage.getItem('userMsg')).App.booklists
      this.imageURL = JSON.parse(sessionStorage.getItem('userMsg')).App.imageURL
  ...
  methods: {
    set (word, keyStr) {
      ...
      var encrypted = this.CryptoJS.AES.encrypt(srcs, key, {mode: this.CryptoJS.mode.ECB, padding: this.CryptoJS.pad.Pkcs7})
    ...
    get (word, keyStr) {
      keyStr = keyStr || 'zyh666isthebest!'
      var key = this.CryptoJS.enc.Utf8.parse(keyStr)// Latin1 w8m31+Yy/Nw6thPsMpO5fg==
      var decrypt = this.CryptoJS.AES.decrypt(word, key, {mode: this.CryptoJS.mode.ECB, padding: this.CryptoJS.pad.Pkcs7})
          ...
          if (response.data.error_num === -2) {
            ...
            if (res.error_num === 0) {
              ...
              for (let j = 0, len = this.booklist.length; j < len; j++) {
                this.msg.push({ 'value': this.booklist[j]['fields']['book_name'] })
              }
      ...
      if (this.user.is_login) {
        if (Array.isArray(input)) {
          ...
          for (let j = 0, len = input.length; j < len; j++) {
            books[input[j]['fields']['book_name']] = input[j]['fields']['is_upload']
          }
          this.$axios({
            method: 'post',
