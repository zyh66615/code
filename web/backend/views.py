...
import os
from backend.tasks import crawl, crawl_images
import time

MEDIA_DIR = 'http://127.0.0.1:8904/media/'
BOOK_DIR = 'http://127.0.0.1:8904/media/books/'
...
def check(func):
    def wrapper(*args, **kwargs):
        ...
        if cache.get(data.get('name')) != 1:
            ...
            response["time"] = str(
                round((time.time() - start) * 1000, 1)) + 'ms'
...
def download(request):
    ...
    flag = Book.objects.filter(book_name=data.get(
        'book_name'), user_name=user).values('download')[0]['download']
    if flag == 1:
        upload = Book.objects.filter(book_name=data.get('book_name'), user_name=user).values('is_upload')[0][
            'is_upload']
        if upload == 1:
            name = Book.objects.filter(book_name=data.get(
                'book_name'), user_name=user).values('title')[0]['title']
            response["url"] = BOOK_DIR + '上传/' + \
                data.get('name') + '/' + name + '.txt'
            response["error_num"] = 1
        else:
            book_state = \
            ...
            name = Book.objects.filter(book_name=data.get(
                'book_name'), user_name=user).values('title')[0]['title']
            if book_state == 1:
                response["url"] = BOOK_DIR + '完整/' + \
                    data.get('book_name') + '/' + name + '.txt'
                response["error_num"] = 1
            else:
                response["url"] = BOOK_DIR + '不完整/' + \
                    data.get('book_name') + '/' + name + '.txt'
                response["error_num"] = 1
    else:
        response["msg"] = '未能爬取该书'
...
def add_book(request):
    ...
    response = {}
    data = json.loads(request.body)
    book = Book.objects.filter(book_name=data.get(
        'book_name'), user_name=data.get('name'))
    if book.exists():
        response["msg"] = '已存在该书'
        response["error_num"] = 1
    else:
...
def reply(request):
    ...
    comment = Comment(name=data.get('name'), answer_post=p,
    ...
    if c.exists():
        for i in j:
            ...
            if k.exists():
                for K in k:
                    reply.append({'name': K.name, 'content': K.content,
                                  'time': format_time(K.add_time), 'id': K.id})
...
def post(request):
    ...
    response = {}
    data = json.loads(request.body)
    p = Post(user_name=data.get('name'), title=data.get(
        'title'), content=data.get('input'))
    p.save()
    response["msg"] = 'success'
    response["error_num"] = 0
    response["time"] = str(round((time.time() - start) * 1000, 1)) + 'ms'
    ...
    for b in books:
        if os.path.exists('./media/books/完整/' + b.book_name):
            ...
            if os.path.exists('./media/books/简介/' + b.book_name + '.txt'):
                brief = open('./media/books/简介/' + b.book_name +
                             '.txt', encoding='utf-8').read()
            b.download = 1
            b.book_state = 1
            b.title = t[0][:-4]
            b.intro = brief
            Book.objects.filter(id=b.id).update(
                download=1, book_state=1, title=t[0][:-4], intro=brief)
    ...
    if p.exists():
        ...
        if c.exists():
            for i in j:
                ...
                if k.exists():
                    for K in k:
                        reply.append({'name': K.name, 'content': K.content, 'time': format_time(
                            K.add_time), 'id': K.id})
                    i['fields']['Reply'] = reply
                else:
                    i['fields']['Reply'] = reply
    ...
    if isinstance(data.get('book_name'), str):
        ...
        for book_name in Books:
            book = Book.objects.filter(
                book_name=book_name, user_name=user_name, is_upload=Books[book_name])
            if book.exists():
                if book.values('is_upload')[0]['is_upload'] == 1:
                    p = './media/books/上传/' + user_name + '/' + book_name + '.txt'
                    if os.path.exists(p):
                ...
                for b in books:
                    if os.path.exists('./media/books/完整/' + b.book_name):
                        ...
                        if os.path.exists('./media/books/简介/' + b.book_name + '.txt'):
                            brief = open(
                                './media/books/简介/' + b.book_name + '.txt', encoding='utf-8').read()
                        b.download = 1
                        b.book_state = 1
                        b.title = t[0][:-4]
                        b.intro = brief
...
def user_login(request):
    ...
    try:
        ...
        if cache.get(data.get('name')) == 1:
            ...
            user = User.objects.filter(name=data.get(
                'name'), password=data.get('password'))
            if user.exists():
                response["msg"] = 'success'
                response["error_num"] = 0
...
def upload_image(request):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if pic.exists():
                pic.delete()
                os.remove('./media/images/' +
                          request.POST.get('name') + '.jpg')
            p = Pic(pic_name=request.POST.get('name'),
                    pic_img=request.FILES.get('file'))
            p.save()
            response["msg"] = 'success'
            response["error_num"] = 0
            response["time"] = str(
...
def upload_text(request):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            b = Book.objects.filter(book_name=request.POST.get(
                'book_name'), user_name=user, is_upload=1)
            if not b.exists():
                book = Book(book_name=request.POST.get('book_name'), user_name=user, is_upload=1,
                            download=1, title=request.POST.get('book_name'), book_state=1, intro='上传的书无简介')
                book.save()
            response["msg"] = 'success'
            response["error_num"] = 0
            response["time"] = str(
